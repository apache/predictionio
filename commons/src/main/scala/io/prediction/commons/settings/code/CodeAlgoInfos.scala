  ...
  private val wipAlgoInfos = Map(
    ...
    "mahout-itembased" -> AlgoInfo(
      ...
      batchcommands = Some(Seq(
        ...
        "hadoop jar $mahoutJar$ org.apache.mahout.cf.taste.hadoop.item.RecommenderJob --input $dataFilePrefix$ratings.csv --output $algoFilePrefix$predicted.tsv --tempDir $mahoutTempDir$ --similarityClassname $similarityClassname$ --numRecommendations 500",
    ...
    "mahout-parallelals" -> AlgoInfo(
      ...
      batchcommands = Some(Seq(
        ...
        "hadoop jar $mahoutJar$ org.apache.mahout.cf.taste.hadoop.als.RecommenderJob --input $algoFilePrefix$matrix/userRatings --userFeatures $algoFilePrefix$matrix/U --itemFeatures $algoFilePrefix$matrix/M --output $algoFilePrefix$predicted.tsv --tempDir $mahoutTempDir$ --numRecommendations 500 --maxRating 5",
        "hadoop jar $pdioJar$ io.prediction.algorithms.mahout.itemrec.itembased.ModelConstructor --hdfs --dbType $modeldataDbType$ --dbName $modeldataDbName$ --dbHost $modeldataDbHost$ --dbPort $modeldataDbPort$ --hdfsRoot $hdfsRoot$ --appid $appid$ --engineid $engineid$ --algoid $algoid$ --modelSet $modelset$")),
      offlineevalcommands = Some(Seq(
        "../lib/quiet.sh hadoop fs -rmr $mahoutTempDir$",
        "../lib/quiet.sh hadoop fs -rmr $algoDir$",
