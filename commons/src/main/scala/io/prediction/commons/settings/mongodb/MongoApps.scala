...
class MongoApps(db: MongoDB) extends Apps {
  ...
  def updateAppkeyByAppkeyAndUserid(appkey: String, userid: Int, newAppkey: String) = {
    appColl.findAndModify(MongoDBObject("appkey" -> appkey, "userid" -> userid), $set("appkey" -> newAppkey)) map { dbObjToApp(_) }
  }

  def updateTimezoneByAppkeyAndUserid(appkey: String, userid: Int, timezone: String) = {
    appColl.findAndModify(MongoDBObject("appkey" -> appkey, "userid" -> userid), $set("timezone" -> timezone)) map { dbObjToApp(_) }
  }
