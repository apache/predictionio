...
trait AbstractEvaluator {
  def getParamsSetBase(params: BaseEvaluationParams): Seq[(BaseTrainingDataParams, BaseEvaluationDataParams)]
  def evaluateBase(feature: BaseFeature,
    predicted: BaseTarget, actual: BaseTarget): Unit
  def report(): Unit
}

trait BaseEvaluator[-EP, +TDP <: BaseTrainingDataParams, +EDP <: BaseEvaluationDataParams, -F, -T] extends AbstractEvaluator {
  override def getParamsSetBase(params: BaseEvaluationParams) = getParamsSet(params.asInstanceOf[EP])
  def getParamsSet(params: EP): Seq[(TDP, EDP)]

  override def evaluateBase(feature: BaseFeature,
    predicted: BaseTarget, actual: BaseTarget) =
    evaluate(feature.asInstanceOf[F],
...
trait BaseDataPreparator[-TDP, +TD <: BaseTrainingData]
  ...
  override def prepareTrainingBase(params: BaseTrainingDataParams) = prepareTraining(params.asInstanceOf[TDP])
  def prepareTraining(params: TDP): TD
}

trait AbstractEvaluationPreparator {
  def prepareEvaluationBase(params: BaseEvaluationDataParams): Seq[(BaseFeature, BaseTarget)]
}

trait BaseEvaluationPreparator[-EDP, +F <: BaseFeature, +T <: BaseTarget]
    extends AbstractEvaluationPreparator {
  override def prepareEvaluationBase(params: BaseEvaluationDataParams) = prepareEvaluation(params.asInstanceOf[EDP])
...
trait BaseAlgorithm[-TD, -F, +T <: BaseTarget, M <: BaseModel, AP <: BaseAlgoParams]
    extends AbstractAlgorithm {
  override def initBase(baseAlgoParams: BaseAlgoParams) =
    init(baseAlgoParams.asInstanceOf[AP])
  def init(algoParams: AP): Unit = {}

  override def trainBase(trainingData: BaseTrainingData): BaseModel = train(trainingData.asInstanceOf[TD])
  def train(trainingData: TD): M

  override def predictBase(baseModel: BaseModel, feature: BaseFeature): BaseTarget =
...
trait BaseServer[-F, T <: BaseTarget] extends AbstractServer {
  override def combineBase(feature: BaseFeature, targets: Seq[BaseTarget]) = combine(feature.asInstanceOf[F], targets.map(_.asInstanceOf[T]))
