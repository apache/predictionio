...
install_hadoop () {
	...
	curl -o apache_mirrors.txt http://www.apache.org/dyn/closer.cgi/hadoop/common/hadoop-1.0.4/hadoop-1.0.4.tar.gz
	HADOOP_URL=$(cat apache_mirrors.txt | grep -m 1 "<strong>.*</strong>" | sed 's/.*<strong>//' | sed 's/<\/strong>.*//')
