...
In this tutorial, we will build a simple **Java single machine recommendation algorithm ** to demonstrate how you could develop your own algorithm and prediction engine with PredictionIO. [MovieLens 100k](http://grouplens.org/datasets/movielens/) data set will be used.
...
Before you deploy algorithm with PredictionIO workflow for deployment, you may want to run some tests to go through the data pipeline step by step to make sure things are working fine.
...
You can find how JavaSimpleEngineBuilder and JavaEngineParamsBuilder is used in Runner.1java:
...
The Runner1 takes the filePath of the rating file as argument. Execute the following command to run:

```
bin/pio-run io.prediction.engines.java.recommendations.Runner1 data/test/ratings.csv
```
...
```
bin/pio-run io.prediction.engines.java.recommendations.Runner2 data/test/ratings.csv
```
