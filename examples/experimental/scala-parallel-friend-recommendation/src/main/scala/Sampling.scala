...
object Sampling {
  ...
  def nodeSampling(sc: SparkContext, graph: Graph[Int, Int], fraction: Double) : Graph[Int, Int] = {
    val vertices = graph.vertices.sample(false, fraction, Random.nextLong)
    val vertexMap = vertices.collectAsMap()
    val edges = graph.edges.filter(e=>vertexMap.contains(e.srcId) && vertexMap.contains(e.dstId))
