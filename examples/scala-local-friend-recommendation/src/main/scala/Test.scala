...
object Test {
  def main(args: Array[String]) {
    ...
    val ds = new FriendRecommendationDataSource(new FriendRecommendationDataSourceParams("data/mini_item.txt", "data/mini_user_key_word.txt", "data/mini_user_action.txt"))
    val td = ds.readTraining()
    val algo = new SimRankAlgorithm(new FriendRecommendationAlgoParams(0.5))
