...
object SimpleApp {
  def main(args: Array[String]) {
    ...
    // trainig data
    val tdMap: Map[Int, BaseTD] = tdpMap.map { case (idx, tdp) =>
      (idx, prepareTraining(tdp).asInstanceOf[TD])
    }

    // model
    val mMap: Map[Int, M] = tdMap.map { case (idx, td) =>
      (idx, train(td.asInstanceOf[TD]))
    }
  ...
  // NOTE:
  def prepareTraining(params: TDP)(implicit sc: SparkContext): BaseTD = {
