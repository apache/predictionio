...
trait BaseEvaluator[
  ...
  -F, -T] extends AbstractEvaluator {
  override def getParamsSetBase(params: BaseEvaluationParams)
...
extends AbstractDataPreparator {
  override def prepareTrainingBase(params: BaseTrainingDataParams)
...
extends AbstractEvaluationPreparator{
  override def prepareEvaluationBase(params: BaseEvaluationDataParams)
    = prepareEvaluation(params.asInstanceOf[EDP])
...
trait BaseAlgorithm[-TD, -F, +T <: BaseTarget, 
...
extends AbstractAlgorithm {
  ...
  override def predictBase(baseModel: BaseModel, feature: BaseFeature)
  : BaseTarget = 
