...
class ModelConstructorTest extends Specification with TupleConversions {
  "itemrec.knnitembased ModelConstructor in test mode" should {
    ...
    val algoid = 7
    val modelSet = true
    val test1ItemRecScores = List(("u0", "i1", "0.123"), ("u0", "i2", "0.456"))
    val test1Items = List(("i0", "t1,t2,t3"), ("i1", "t1,t2"), ("i2", "t2,t3"))
    val test1Output = List(("u0", "i1", 0.123, "t1,t2", algoid, modelSet), ("u0", "i2", 0.456, "t2,t3", algoid, modelSet))
    ...
    JobTest("io.prediction.algorithms.scalding.itemrec.knnitembased.ModelConstructor")
      ...
      .sink[(String, String, Double, String, Int, Boolean)](ItemRecScores(dbType = dbType, dbName = dbName, dbHost = dbHost, dbPort = dbPort).getSource) { outputBuffer =>
